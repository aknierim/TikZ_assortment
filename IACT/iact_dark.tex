...
\begin{document}
  \begin{tikzpicture}
    ...
    \node[xshift=\globalshift+1cm] at (8,2.3) {\reflectbox{\includegraphics[height=0.5cm]{subgraphics/xkcd_dark.pdf}}};
    ...
    % CHERENKOV RADIATION
    \node [] at (-2.3, 6) {\includegraphics[height=5.7cm]{subgraphics/cherenkov_radiation_dark.pdf}};


    % INITIAL GAMMA
    \draw[yellow, xshift=\globalshift] (-0.65,9) -- (0.1,7.85) node[midway, above right, maincolour] {\large $\gamma$-ray entering the atmosphere};

    % EXTENSIVE AIR SHOWER
    \fill[opacity=1, path fading=fadeEAS, left color=yellow, right color=yellow, xshift=\globalshift] (0,8) -- (1,6) arc(30:200:-0.35) -- cycle;

    % HEITLER MODEL
      \node [] at (-2.3, 6) {\includegraphics[height=5.7cm]{subgraphics/heitler_model_dark.pdf}};


    % TELESCOPES; Image Credits: Kai Brügge
    \node[inner sep=15pt, xshift=\globalshift] at (6,1.3){\reflectbox{\includegraphics[height=2.5cm]{subgraphics/contours_mst_light.pdf}}};
    \node[inner sep=15pt, xshift=\globalshift] at (4,1.3){\reflectbox{\includegraphics[height=2.5cm]{subgraphics/contours_mst_light.pdf}}};
    ...
    % CAMERA FRAME
    \node [circle, draw, color=maincolour, fill=darkmode, minimum width = 4cm, path picture = {
      \node [] at (path picture bounding box.center) {\includegraphics[height=4.4cm]{subgraphics/mst_camera_frame.pdf}};
      }, xshift=\globalshift] (CAMERAFRAME) at (-3,3.5) {};

    \node [above, yshift=2.1cm] at (CAMERAFRAME) {\large Snapshot in camera frame};
